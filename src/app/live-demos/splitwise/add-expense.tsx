...
import { Box, TextField, Button, MenuItem, InputLabel, Select, FormControl, Radio, RadioGroup, FormControlLabel, OutlinedInput, Chip } from "@mui/material";
...
function getStyles(name: string, memberName: string, theme: any) {
    return {
        fontWeight:
...
export default function AddExpense(props: any) {
    ...
    const handleRadioChange = (event: any, index: number) => {
        ...
        newExpense[index].splitWith = members
            .filter((m: any) => m.name !== newExpense[index].paidBy)
            .map((m: any) => m.name);
                            ...
                            >
                                ...
                                >
                                    {members?.map((member: any, index: number) => {
                                        ...
                                        return (
                                            <MenuItem value={member.name} key={index}>{member.name}</MenuItem>
                                        );
                                    })}
                                </Select>
                            </FormControl>
                        ...
                        {expense.split == "specific" && (
                            <div className="row">
                                ...
                                <FormControl sx={{ m: 1, width: 300 }}>
                                    ...
                                    >
                                        {members
                                            .filter((m: any) => m.name != expense?.paidBy)
                                            .map((m: any) => (
                                                <MenuItem
                                                    key={m.name}
                                                    value={m.name}
